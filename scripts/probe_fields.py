...
# 🔴 **범위로 쓰면 안 된다.** `발행년도,A|발행년도,B` 는 [A,B] 안에서 **최신순**으로 주므로
#    page 1 만 받으면 표본이 **상한 연도 B 한 해**로 수렴한다(적대적 리뷰 실측:
#    `1960|1999` 로 1,000건을 받으면 학위년도가 전건 1999). 즉 초판의 '3개 구간'은
#    실제로는 '3개 연도'였다. → 단일 연도 앵커로 **정직하게** 표기한다.
STRATA = [
    ("최신(무필터)", None),
    ("2010년", "발행년도,2010|발행년도,2010"),
    ("1989년", "발행년도,1989|발행년도,1989"),
]
