...
| `na_collect` | 검색어 합집합 수집 → xlsx/csv/json/sqlite. `toc_max>0` 이면 **목차 본문 보강**(기본 0=끔, 건당 1회, 권장 300). 본문은 json·sqlite 로만, 표에는 `toc_status`·`toc_chars` |
...
- ✅ **공개·릴리스 완료** — v0.1.1, MCP 레지스트리 등록, 사용량 자동 기록.
- 🔴 **탐침 계측기 자체의 결함 3종 (2026-09-08, 회귀 170 → 185)**
  설계 워크플로가 코드를 읽다 잡았고, 전부 **오프라인으로 재현**했다.
  ① `probe_limits._candidate_pool` 이 `DB_CATEGORIES` 를 임포트하지 않아 **NameError 로
     죽어 있었다**(그 이름은 `probe_categories()` 안에서만 지역 임포트됐다).
     즉 `probe_limits.py categories` 는 호출 즉시 터졌고, **'후보 풀 광역화'는 한 번도
     실행된 적이 없다.** §F-3 이 `DB_CATEGORIES` 를 "전수 실측·재현 가능"이라 적어 둔
     근거가 이 경로인데 그 경로가 끊겨 있었다.
  ② `_category_ok` 가 **종결코드(쿼터 22·키 30·31)를 '항목 거부'로 기록**했다.
     쿼터가 끊긴 시점 이후의 후보 전부가 거부로 확정되고 그 표가 화이트리스트가 된다.
     → `ProbeAborted` 로 즉시 중단하고 **어디까지 측정했는지**를 출력한다.
  ③ `probe_discover.classify` 가 망·파싱 실패까지 '거부'로 접었다.
     → '측정실패' 판정을 신설해 분리했다. **'측정 못 함'과 '거부됨'은 처방이 다르다.**
  🔴 ①은 §6-A 가 경고한 실패 양식(재현 경로가 끊겼는데 문서는 '실측'이라 말함)이 **문서가
     아니라 내 탐침에서** 재발한 것이다.
- 🔴 **`parse_toc_response` 에만 `resultCode` 검사가 없었다** — 검색·상세 파서에는 있다.
  `_check_error_envelope` 는 게이트웨이 봉투만 보므로, 루트가 `<response>` 인 채 코드만
  비정상인 응답(실측 표본 `ECHO_KEY_ERR`=10)은 통과했고 `<toc>` 가 없어
  **오류가 '목차 없음'으로 조용히 바뀌었다.** 대량 보강에서는 실패 전건이 '목차 없는 자료'로
  굳는다 — 이 저장소가 막겠다고 한 실패 양식 그대로다. 회귀 3건으로 고정.
- ✅ **`na_collect` 목차 보강 완료 (2026-09-08, 회귀 185 → 209)** — `toc_max` 정수 1개, 기본 0(끔).
  설계 3안을 심사 2인이 갈라(쿼터안전 vs 연구흐름) **연구흐름을 뼈대로 쿼터안전의 장치를 접붙였다**.
  결정적 산수: 1안은 재시도(×2)를 예약해 100건에 200호출, 3안은 **건당 1회로 결정론화**해
  300건에 300호출 — 도달 가능한 최악값은 같은데 수율이 3배다. 재시도를 걷어낸 대신 실패를
  **감추지 않고 보고**한다(`failed` + 제어번호 → `na_toc` 단건 재조회).
  🔴 이 기능의 위험은 **부분 성공이 전체 성공으로 읽히는 것**이라, 목차가 안 붙는 다섯 사유를
     `toc_status` 한 컬럼에서 다른 값으로 보이게 했다:
     `skipped`(플래그가 Y 아님) · `empty`(플래그가 거짓) · `sentinel` · `failed` · `not_attempted`.
     경고도 처방별로 셋으로 나눴다 — 예산 초과(=상향으로 해결) / 조회 실패(=상향 무의미) /
     중단(=쿼터·키 문제라 상향은 악화).
  · 예산은 검색과 **별도**(`MAX_TOC_CALLS_PER_TOOL_CALL=300`) — 합산하면 상한을 올리는 순간
    검색 가드까지 풀린다. 대가로 한 호출 최악값이 300 → 600(일일 6%)이 된다는 것을 명시해 둔다.
  · 본문은 **json·sqlite 로만** 나간다(xlsx 셀 상한 32,767·csv 비대화·컨텍스트 소모).
    요청 형식에 둘 다 없으면 `toc_enrich_output_note` 로 경고한다.
- 🔴 **별건 — 같은 결함을 한 곳만 고쳐 뒀다**: `search_meta` 는 회수 한계를 `PAGENO_MAX×page_size`
  로 계산하는데 `search_terms_meta`(합집합 경로)만 상수 `API_RECORD_CAP` 을 실었다.
  `page_size` 를 낮추면 **거짓 보고**가 된다. 계산값으로 교체 + 회귀 3건.
- 🔴 **탐침 비용**: `classify` 가 거부 1건당 **9회**(검색어 3 × 재시도 3)를 태웠다. 항목 거부는
  검색어와 무관하다는 것이 이미 실측인데도 그랬다 → **3회**로. 표시 전용 이름은 후보에서 제외
  (조용히 빼지 않고 몇 개를 왜 뺐는지 출력한다).
- ⏭️ 다음: ① 화이트리스트 재측정 — NameError 를 고쳤으니 **광역화된 풀로 처음 돌려볼 수 있다.**
     ⚠️ 아래 서술은 **틀렸었다**(2026-09-08 정정): "'목록에 없는데 동작하는 이름'
     (국회회의록 `발행자`)은 원리상 못 찾는다." → `발행자` 는 `_BOOKISH` 에 있어 타 자료종
     어휘 광역화로 **이미 후보에 들어온다**. 게다가 `EMPTY_FIELDS_BY_DB["국회회의록"]` 의
     `발행자` 는 검색항목이 아니라 **응답 필드**였다 — 예시 자체가 범주 혼동이었다.
     `probe_discover.py` 는 아예 관측 필드명을 후보로 쓴다. 남는 진짜 한계는
     **"어떤 dbname 어휘에도 없고 응답에도 안 나타나는 이름"** 뿐이다.
